...
macro_rules! make_limited_int_setter(
    ($attr:ident, $htmlname:tt, $default:expr) => (
        fn $attr(&self, value: i32) -> $crate::script::dom::bindings::error::ErrorResult {
            use $crate::script::dom::bindings::inheritance::Castable;
            use $crate::script::dom::element::Element;
            use $crate::script::script_runtime::CanGc;

            let value = if value < 0 {
                return Err($crate::script::dom::bindings::error::Error::IndexSize(None));
            } else {
                value
            };
...
macro_rules! make_form_action_getter(
    ( $attr:ident, $htmlname:tt ) => (
        fn $attr(&self) -> DOMString {
            ...
            use $crate::script::dom::element::Element;
            let element = self.upcast::<Element>();
            let doc = $crate::script::dom::node::NodeTraits::owner_document(self);
...
macro_rules! make_uint_setter(
    ($attr:ident, $htmlname:tt, $default:expr) => (
        fn $attr(&self, value: u32) {
            use $crate::script::dom::bindings::inheritance::Castable;
            use $crate::script::dom::element::Element;
            use $crate::script::dom::values::UNSIGNED_LONG_MAX;
            use $crate::script::script_runtime::CanGc;
...
macro_rules! make_limited_uint_setter(
    ($attr:ident, $htmlname:tt, $default:expr) => (
        fn $attr(&self, value: u32) -> $crate::script::dom::bindings::error::ErrorResult {
            use $crate::script::dom::bindings::inheritance::Castable;
            use $crate::script::dom::element::Element;
            use $crate::script::dom::values::UNSIGNED_LONG_MAX;
            use $crate::script::script_runtime::CanGc;
...
macro_rules! unsafe_no_jsmanaged_fields(
    ($($ty:ty),+) => (
        $(
            #[expect(unsafe_code)]
            unsafe impl $crate::script::dom::bindings::trace::JSTraceable for $ty {
                #[inline]
...
macro_rules! event_handler(
    ($event_type: ident, $getter: ident, $setter: ident) => (
        define_event_handler!(
            crate::script::dom::bindings::codegen::Bindings::EventHandlerBinding::EventHandlerNonNull,
            $event_type,
            $getter,
            $setter,
...
macro_rules! error_event_handler(
    ($event_type: ident, $getter: ident, $setter: ident) => (
        define_event_handler!(
            crate::script::dom::bindings::codegen::Bindings::EventHandlerBinding::OnErrorEventHandlerNonNull,
            $event_type,
            $getter,
            $setter,
...
macro_rules! beforeunload_event_handler(
    ($event_type: ident, $getter: ident, $setter: ident) => (
        define_event_handler!(
            crate::script::dom::bindings::codegen::Bindings::EventHandlerBinding::OnBeforeUnloadEventHandlerNonNull,
            $event_type,
            $getter,
            $setter,
...
macro_rules! hppr_dispatch {
    ...
    (@body $self_:ident, $can_gc:ident, ($($env:tt)*), $do_method:ident, $($do_arg:expr),* ) => {{
        ...
        let cb = $crate::script::routed_promise::callback_promise::<HpprProtocolResponse, _>(
    ...
    ($method:ident, can_gc, ($($env:tt)*), $do_method:ident, $($do_arg:expr),* ; $($arg:ident : $argty:ty),+) => {
        fn $method(&self, $($arg : $argty,)+ can_gc: CanGc) -> ::std::rc::Rc<$crate::script::dom::promise::Promise> {
    ...
    ($method:ident, ($($env:tt)*), $do_method:ident, $($do_arg:expr),* ; $($arg:ident : $argty:ty),+) => {
        fn $method(&self, $($arg : $argty),+) -> ::std::rc::Rc<$crate::script::dom::promise::Promise> {
    ...
    ($method:ident, ($($env:tt)*), $do_method:ident $(,)?) => {
        fn $method(&self) -> ::std::rc::Rc<$crate::script::dom::promise::Promise> {
            let can_gc = CanGc::note();
