...
use super::cell::{ArcRefCell, WeakRefCell};
use super::context::LayoutContext;
use super::dom_traversal::{Contents, NodeAndStyleInfo};
use super::flexbox::FlexLevelBox;
use super::flow::inline::{InlineItem, SharedInlineStyles, WeakInlineItem};
use super::flow::{BlockLevelBox, BlockLevelCreator};
use super::fragment_tree::FragmentFlags;
use super::geom::PhysicalSize;
use super::layout_box_base::LayoutBoxBase;
use super::replaced::{CanvasInfo, IFrameInfo, ImageInfo, VideoInfo};
use super::style_ext::{
    ComputedValuesExt, Display, DisplayGeneratingBox, DisplayLayoutInternal, DisplayOutside,
};
use super::table::{TableLevelBox, WeakTableLevelBox};
use super::taffy::TaffyItemBox;
...
impl WeakLayoutBox {
    pub(super) fn upgrade(&self) -> Option<LayoutBox> {
        Some(match self {
            Self::DisplayContents(inline_shared_styles) => {
...
pub struct BoxSlot<'dom> {
    pub(super) slot: ArcRefCell<Option<LayoutBox>>,
    pub(super) marker: PhantomData<&'dom ()>,
}
...
impl BoxSlot<'_> {
    pub(super) fn set(self, layout_box: LayoutBox) {
        layout_box.attached_to_tree(layout_box.downgrade());
        *self.slot.borrow_mut() = Some(layout_box);
    }

    pub(super) fn take_layout_box(&self) -> Option<LayoutBox> {
...
pub(in crate::layout) trait NodeExt<'dom> {
