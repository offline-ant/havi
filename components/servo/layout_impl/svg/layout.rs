...
use crate::layout::geom::{LogicalVec2, PhysicalPoint, PhysicalRect, PhysicalSize};
use crate::layout::layout_box_base::{CacheableLayoutResult, LayoutBoxBase};
use crate::layout::positioned::PositioningContext;
use crate::layout::sizing::{
    ComputeInlineContentSizes, InlineContentSizesResult, LazySize, SizeConstraint,
};
use crate::layout::style_ext::{AspectRatio, Clamp, ComputedValuesExt};
use crate::layout::{ConstraintSpace, ContainingBlock};
...
fn build_svg_node_fragment(
    ...
    match (&node.summary.kind, &node.svg_data().node_kind, &node.resolved_style) {
        (
            ...
            Some(Fragment::SVGViewport(crate::layout::cell::ArcRefCell::new(
                SVGViewportFragment {
...
fn build_svg_child_fragment(
    ...
    match (&node.summary.kind, &node.svg_data().node_kind, &node.resolved_style) {
        (SVGLayoutNodeKind::Group, _, SVGNodeResolvedStyle::Geometry(_style)) => {
            ...
            Some(Fragment::SVGContainer(crate::layout::cell::ArcRefCell::new(
        ...
        (
            ...
            Some(Fragment::SVGLeaf(crate::layout::cell::ArcRefCell::new(SVGLeafFragment {
                base: BaseFragment::new(
                    base_fragment_info,
                    node.computed_style.clone().into(),
...
mod tests {
    ...
    use super::fragment_tree::Fragment;
    use super::geom::{PhysicalPoint, PhysicalSize};
    use super::svg::parse::parse_svg_tree;
    use super::svg::style as svg_style;
    use super::svg::tree::{SVGNodeData, SVGNodeId, SVGStandaloneTree, SVGTreeChild, SVGTreeNode};
        ...
        match (&node.summary.kind, &node.svg_data().node_kind, &node.resolved_style) {
            (
                ...
                let children = node
                    ...
                    .collect();
                Some(Fragment::SVGViewport(crate::layout::cell::ArcRefCell::new(SVGViewportFragment {
                    base: BaseFragment::new(base_fragment_info, style.into(), rect),
                    identity: SVGFragmentIdentityContext::default().fragment_identity(node.tag),
                    children,
                    viewport_rect,
        ...
        match (&node.summary.kind, &node.svg_data().node_kind, &node.resolved_style) {
            (SVGLayoutNodeKind::Group, _, SVGNodeResolvedStyle::Geometry(_)) => {
                let children = node
                    ...
                    .collect::<Vec<_>>();
                let rect = union_fragment_rects(&children);
                Some(Fragment::SVGContainer(crate::layout::cell::ArcRefCell::new(SVGContainerFragment {
                    base: BaseFragment::new(base_fragment_info, node.computed_style.clone().into(), rect),
                    identity,
                    kind: SVGContainerKind::Group,
                    children,
