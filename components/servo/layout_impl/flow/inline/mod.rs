...
use crate::layout::cell::{ArcRefCell, WeakRefCell};
use crate::layout::context::LayoutContext;
use crate::layout::dom::WeakLayoutBox;
use crate::layout::dom_traversal::NodeAndStyleInfo;
use crate::layout::formatting_contexts::{Baselines, IndependentFormattingContext};
use crate::layout::fragment_tree::{
    BoxFragment, CollapsedMargin, Fragment, FragmentFlags, PositioningFragment,
};
use crate::layout::geom::{LogicalRect, LogicalSides1D, LogicalVec2, ToLogical};
use crate::layout::layout_box_base::LayoutBoxBase;
use crate::layout::positioned::{AbsolutelyPositionedBox, PositioningContext};
use crate::layout::sizing::{
    outer_inline, ComputeInlineContentSizes, ContentSizes, InlineContentSizesResult,
};
use crate::layout::style_ext::{ComputedValuesExt, PaddingBorderMargin};
use crate::layout::{ConstraintSpace, ContainingBlock, IndefiniteContainingBlock, SharedStyle};
use super::float::{Clear, FloatBox, PlacementAmongFloats, SequentialLayoutState};
use self::line::TextRunOffsets;
use super::{
    layout_in_flow_non_replaced_block_level_same_formatting_context, BlockContainer,
    CacheableLayoutResult, CollapsibleWithParentStartMargin, FloatSide,
    IndependentFloatOrAtomicLayoutResult, PlacementState,
};
...
impl InlineItem {
    ...
    pub(in crate::layout) fn with_base<T>(&self, callback: impl FnOnce(&LayoutBoxBase) -> T) -> T {
        match self {
    ...
    pub(in crate::layout) fn with_base_mut<T>(&self, callback: impl FnOnce(&mut LayoutBoxBase) -> T) -> T {
        match self {
    ...
    pub(in crate::layout) fn attached_to_tree(&self, layout_box: WeakLayoutBox) {
        match self {
