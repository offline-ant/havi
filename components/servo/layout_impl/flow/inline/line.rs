...
use crate::layout::fragment_tree::{BaseFragment, BaseFragmentInfo, BoxFragment, Fragment, TextFragment};
...
use crate::layout::{ContainingBlock, ContainingBlockSize};

pub(super) struct LineMetrics {
    /// The block offset of the line start in the containing
    /// [`super::flow::InlineFormattingContext`].
    pub block_offset: Au,
...
impl LineItemLayout<'_, '_> {
    ...
    fn start_inline_box(&mut self, identifier: &InlineBoxIdentifier) {
        ...
        let parent_offset = LogicalVec2 {
            ...
            block: self.line_metrics.block_offset + block_start_offset,
        };
        ...
        let outer_state = std::mem::replace(
            ...
            LineItemLayoutInlineContainerState::new(
                Some(*identifier),
                parent_offset,
                self.line_metrics.block_offset + block_start_offset + space_above_baseline,
                positioning_context_or_start_offset_in_parent,
            ),
        );
