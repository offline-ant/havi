...
use super::async_runtime::{init_async_runtime, spawn_task};
use super::connector::create_http_client;
use super::cookie::ServoCookie;
use super::cookie_storage::CookieStorage;
use super::embedder::NetToEmbedderMsg;
use crate::net::fetch::cors_cache::CorsCache;
use crate::net::fetch::fetch_params::{FetchParams, SharedPreloadedResources};
use crate::net::fetch::methods::{
    CancellationListener, FetchContext, SharedInflightKeepAliveRecords, fetch,
};
use super::filemanager_thread::FileManager;
use super::hsts::{self, HstsList};
use super::http_cache::HttpCache;
use super::http_loader::{HttpState, http_redirect_fetch};
    ...
    fn process_msg(
        ...
        match msg {
            ...
            CoreResourceMsg::HpprWatch {
                ...
                let hppr_state = self.resource_manager.hppr_state.clone();
                spawn_task(async move {
                    super::watch_loader::start_watch(
            ...
            CoreResourceMsg::HpprStreamPub {
                ...
                let hppr_state = self.resource_manager.hppr_state.clone();
                spawn_task(async move {
                    super::stream_pub_loader::start_stream_pub(
            ...
            CoreResourceMsg::HpprStreamSub {
                ...
                let hppr_state = self.resource_manager.hppr_state.clone();
                spawn_task(async move {
                    super::stream_sub_loader::start_stream_sub(
                        &hppr_state, &endpoint, signer, &prefix,
                        event_sender, action_receiver,
                    ).await;
                });
...
pub struct CoreResourceManager {
    ...
    hppr_state: Arc<super::hppr_pool::HpprAsyncState>,
}
...
impl CoreResourceManager {
    pub fn new(
        ...
        CoreResourceManager {
            ...
            hppr_state: Arc::new(super::hppr_pool::HpprAsyncState::new(hppr_home_target)),
        }
    }
...
use super::hppr_pool::HpprAsyncState;
...
async fn hppr_chunk_reassemble(
    ...
    let reassembled = super::hppr_chunks::batch_reassemble_chunks(hppr_state, endpoint, is_repo, &manifest)
        .await
        .map_err(|e| HpprProtocolError {
            error_type: "CHUNK".to_string(),
            detail: e,
