...
impl Framebuffer {
    ...
    /// `texture_target` is `GL_TEXTURE_2D` or `GL_TEXTURE_RECTANGLE`.
    fn set_external_texture(&mut self, new_texture_id: gl::GLuint, size: PhysicalSize<u32>, texture_target: u32) {
        self.gl
            .bind_framebuffer(gl::FRAMEBUFFER, self.framebuffer_id);
...
impl MakepadRenderingContext {
    ...
    pub unsafe fn new_from_loader(
        ...
        debug!("MakepadRenderingContext (direct context): size={:?} api={:?}", size, gl_api);

        let gleam_gl: Rc<dyn Gl> = unsafe {
            match gl_api {
                GlApi::GL => gl::GlFns::load_with(|name| gl_loader(name)),
                GlApi::GLES => gl::GlesFns::load_with(|name| gl_loader(name)),
            }
        };
        let glow_gl = unsafe {
