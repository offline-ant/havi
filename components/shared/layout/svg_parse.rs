...
impl SVGAttributeMap {
    ...
    fn typed_length(&self, name: &str) -> Option<crate::SVGLengthValue> {
        self.attr(name).map(|raw| crate::parse_svg_length(Some(raw)))
    }
}
            ...
            Token::Tag(tag) if matches!(tag.kind, TagKind::StartTag | TagKind::EmptyTag) => {
                ...
                *self.metadata.borrow_mut() = Some(SVGRootMetadata {
                    viewport: SVGViewportData {
                        ...
                        view_box: crate::parse_svg_optional_view_box(attrs.attr("viewBox")),
                        preserve_aspect_ratio: crate::parse_svg_preserve_aspect_ratio(
                            attrs.attr("preserveAspectRatio"),
                        ),
                        overflow_hidden: crate::parse_svg_overflow_hidden(attrs.attr("overflow")),
                    },
                });
                ProcessResult::Done
            }
