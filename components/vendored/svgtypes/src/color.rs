    ...
    pub fn parse_color(&mut self) -> Result<Color, Error> {
        ...
        if self.curr_byte()? == b'#' {
            ...
            if name == "rgb" || name == "rgba" {
                ...
                let l = self.parse_list_number()?;
                let c = self.parse_list_number()?;
                let h_deg = self.parse_list_number()?;
                let h_rad = h_deg * core::f64::consts::PI / 180.0;

                let ok_a = c * h_rad.cos();
                let ok_b = c * h_rad.sin();
                color = oklab_to_rgb(l, ok_a, ok_b);
                ...
                let l = self.parse_list_number()?;
                let a = self.parse_list_number()?;
                let b = self.parse_list_number()?;
                color = oklab_to_rgb(l, a, b);
...
/// Convert linear RGB channel to sRGB u8.
...
mod tests {
    ...
    test!(
        oklch_red,
        "oklch(0.85 0.14 0)",
        Color::new_rgb(255, 165, 202)
    );

    test!(
        oklch_cyan,
        "oklch(0.85 0.14 210)",
        Color::new_rgb(46, 230, 255)
    );

    test!(
        oklch_with_alpha,
        "oklch(0.85 0.14 0, 0.5)",
        Color::new_rgba(255, 165, 202, 128)
    );
    ...
    test!(
        oklab_gray,
        "oklab(0.5 0 0)",
        Color::new_rgb(99, 99, 99)
    );

    test!(
        oklab_with_alpha,
        "oklab(0.5 0 0, 0.5)",
        Color::new_rgba(99, 99, 99, 128)
    );
}
