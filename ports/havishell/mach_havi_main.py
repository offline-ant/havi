...
# Env vars that mozjs_sys's build.rs (and cc-rs) monitor via
# rerun-if-env-changed.  We pin these in .cargo/config.toml so that
# switching between mach-havi and bare cargo build produces identical
# fingerprints and avoids costly SpiderMonkey rebuilds.
#
# Full list from mozjs_sys ENV_VARS + SM_TARGET_ENV_VARS + cc-rs
# target-qualified variants.  Target-triple variants (e.g.
# CC_x86_64-unknown-linux-gnu) are generated dynamically.
_MOZJS_BASE_VARS = [
    "AR", "ARFLAGS", "AS",
    "BINDGEN_EXTRA_CLANG_ARGS",
    "CC", "CC_ENABLE_DEBUG_OUTPUT", "CC_KNOWN_WRAPPER_CUSTOM",
    "CFLAGS", "CLANGFLAGS", "CLANG_PATH",
    "CPP", "CPPFLAGS", "CRATE_CC_NO_DEFAULTS",
    "CXX", "CXXFLAGS", "CXXSTDLIB",
    "HOST_AR", "HOST_ARFLAGS", "HOST_CXX", "HOST_CXXFLAGS", "HOST_CXXSTDLIB",
    "LIBCLANG_PATH",
    "MAKE",
    "OBJCOPY",
    "PYTHON", "READELF",
    "STLPORT_LIBS",
    "TARGET_AR", "TARGET_AS", "TARGET_CC", "TARGET_CFLAGS",
    "TARGET_CLANGFLAGS", "TARGET_CPP", "TARGET_CPPFLAGS",
    "TARGET_CXX", "TARGET_CXXFLAGS", "TARGET_OBJCOPY", "TARGET_READELF",
    "TARGET_WASI_SDK_PATH", "WASI_SDK_PATH",
]

# cc-rs also emits target-qualified variants for these base names.
_CC_RS_QUALIFIED = [
    "AR", "ARFLAGS", "AS",
    "BINDGEN_EXTRA_CLANG_ARGS",
    "CC", "CFLAGS", "CLANGFLAGS",
    "CPP", "CPPFLAGS",
    "CXX", "CXXFLAGS", "CXXSTDLIB",
    "OBJCOPY", "READELF", "WASI_SDK_PATH",
]


def _mozjs_env_vars(target: str | None = None) -> list[str]:
    """Full list of env vars to pin, including target-qualified cc-rs variants."""
    result = list(_MOZJS_BASE_VARS)
    if target:
        # cc-rs checks both VAR_<triple-with-dashes> and VAR_<triple-with-underscores>
        t_dash = target
        t_under = target.replace("-", "_")
        for var in _CC_RS_QUALIFIED:
            result.append(f"{var}_{t_dash}")
            result.append(f"{var}_{t_under}")
    return result


def _write_cargo_config(env: dict[str, str], target: str | None = None) -> None:
    """Write .cargo/config.toml with [env] pinning build-tool variables.

    Only writes vars that are set in *env*.  Existing config.toml is
    overwritten — it is a generated file.
    """
    cargo_dir = HAVI_ROOT / ".cargo"
    config_path = cargo_dir / "config.toml"

    lines = [
        "# Generated by mach-havi. Do not edit.",
        "# Pins build-tool env vars so mozjs_sys is not rebuilt when",
        "# switching between mach-havi and bare cargo build.",
        "",
        "[env]",
    ]

    for var in _mozjs_env_vars(target):
        val = env.get(var)
        if val is not None:
...
def setup_desktop_env() -> dict[str, str]:
    ...
    # Determine host target triple for target-qualified env var pinning.
    host_target = None
    try:
        result = subprocess.run(
            ["rustc", "-vV"], capture_output=True, encoding="utf-8", timeout=5)
    ...
    _write_cargo_config(env, target=host_target)
    return env
