...
pub(super) fn lower_text_primitive(
    ...
    let (glyph_run_key, glyph_run) = make_html_glyph_run_resource(owner_node_id, bounds, tf, font_key)?;
...
fn make_html_glyph_run_resource(
    ...
    let shadows = computed
        ...
        .map(|shadow| MpTextShadow {
            offset: makepad_widgets::dvec2(shadow.horizontal.px() as f64, shadow.vertical.px() as f64),
...
fn positioned_glyphs(
    ...
    font_size_px: f32,
    baseline_ascent: f32,
    glyphs: &[havi_types::fragment_tree::ShapedGlyph],
) -> Result<(Vec<MpPositionedGlyph>, f32), String> {
    let mut pen_x = 0.0_f64;
    let mut advance_width = 0.0_f32;
    let glyphs = glyphs
        .iter()
        .map(|glyph| {
            let origin = makepad_widgets::dvec2(
